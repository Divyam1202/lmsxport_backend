...
export const updateProgress = async (req, res) => {
    ...
    try {
        ...
        const isEnrolled = course.students.some((student) => student.toString() === studentId);
        if (!isEnrolled) {
            return res.status(403).json({
                success: false,
                message: "You are not enrolled in this course",
        ...
        const progressIndex = course.progress.findIndex((entry) => entry.student.toString() === studentId);
        if (progressIndex >= 0) {
...
export const viewTeacherCourses = async (req, res) => {
